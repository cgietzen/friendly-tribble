...
import { FormControl, FormLabel, Textarea, Input, Button, VStack, Text, Center, Collapse } from "@chakra-ui/react";
...
export default function Contact({toggle}) {
    ...
    return (
        ...
        <div sx={{ '--black': '#000000' }} className="w-full last:absolute top-1/2 transform -translate-y-1/2 md:-translate-y-[48vw] grid grid-cols-12">
        <div className="col-start-1 col-span-12 lg:col-start-4 lg:col-span-6 px-8 pb-8 pt-12 shadow-lg bg-white relative">
         <button onClick={toggle} className="absolute top-0 right-0 mt-2 mr-2">
        ...
        <VStack spacing='2rem'>
            ...
            <Button colorScheme='yellow' isLoading={isLoading} disabled={!values.email || !values.message} onClick={onSubmit}>Submit</Button>
        </VStack>
        </Collapse>
        <Collapse in={success} animateOpacity>
        <Center>
